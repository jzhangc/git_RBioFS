...
#' @param n Number of features to plot. Options are \code{"all"} or an integer. Default is \code{"all"}.
...
rbioFS_rf_initialFS <- function(objTitle = "x_vs_tgt",
  ...
  cartprune <- prune(cartTree, cp = mincp) # prune the tree so that SD values that won't impact Rank classification are discarded
  minpredv <- min(predict(cartprune)) # obtain the minimum prediction value (predicted SD) as the SD threshold for Mean
...
rbioFS_rf_initialFS_v2 <- function(objTitle = "x_vs_tgt",
  ...
  # ranked vi dataframe
  outdfm_vi <- data.frame(tmpdfm_vi[order(tmpdfm_vi$Mean, decreasing = TRUE), ],
                          Rank = c(1:nrow(tmpdfm_vi))) # make sure to resort the dataframe in descending order.
