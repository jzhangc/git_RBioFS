...
#' insufficient CPU thread utilization.
...
#' @title rbioClass_svm_ncv_fs_v3
#'
#' @description Nested cross-validation feature selection and SVM modelling: CV-rRF-FS-SVM with sample-ID-aware grouping.
#'   This version ensures samples sharing the same sample ID are kept together in the same fold,
#'   which is necessary when multiple measurements are taken from the same subject/group.
#' @param x Input data matrix (e.g., independent variables, predictors, features, X, etc). Make sure it is either a matrix or a dataframe.
#' @param y Input response variable (e.g., dependent variables, Y etc). Make sure it is \code{factor} class.
#' @param univariate.fs If to use limma-based univariate reduction. Default is \code{FALSE}.
#' @param uni.log2trans Only set if \code{univariate.fs = TRUE}, if to log2 transform data before univariate reduction.
...
#' \code{randomized.sample.index}: randomized sample order for (outer) cross-validation. \code{NULL} for classification modelling (cv.fold already randomized).
#'
#' \code{sampleIds}: the input sample IDs vector, or \code{NULL} if not provided.
#'
#' \code{sampleIdToFold}: a named integer vector mapping each sample ID to its assigned fold (only present when \code{sampleIds} is provided).
#'
#' \code{model.type}: the SVM model type, "classification" or "regression".
#'
#' \code{tot.nested.accuracy.summary}: total (i.e. mean) nested cross-validation accuracy, if \code{model_type = "classification"}
...
#' is assigned to a fold such that the distribution of class labels across folds roughly matches the overall distribution.
#'
#' For regression with \code{sampleIds}, groups are assigned to folds using a variance-minimising approach
#' to balance the covariate distribution across folds. When that is not feasible (too few groups), it falls back
#' to random group-level assignment.
#'
#' For now, RBioFS implementation of two-step random forest feature selection is used to select features based on nested cross-validation.
#' Resulted features from each nested cross-validation round are voted. Features with votes equal or greater than the cutoff are reported as selected features.
#'
...
#' The function also supports regression study, in which case, the performance metric is \code{RMSE}.
#'
#' For classification modelling, the CV segmentation is stratified at the group level (when group-aware), whereas for regression
#' the group-level covariate distribution is balanced via variance minimisation or random assignment.
#'
#'
#' @import foreach
#' @import doParallel
...
#' @examples
#' \dontrun{
#' # Example with independent samples (same as v2):
#' svm_nestedcv <- rbioClass_svm_ncv_fs_v3(x = mydata[, -c(1:2)],
#'                                          y = factor(mydata$Conditions, levels = unique(mydata$Conditions)),
#'                                          center.scale = TRUE,
#'                                          cross.k = 5,
#'                                          sampleIds = NULL)
#'
#' # Example with group-dependent samples (e.g., repeated measures from same subject):
#' svm_nestedcv_grouped <- rbioClass_svm_ncv_fs_v3(x = mydata[, -c(1:2)],
#'                                                   y = factor(mydata$Conditions, levels = unique(mydata$Conditions)),
#'                                                   center.scale = TRUE,
#'                                                   cross.k = 5,
#'                                                   sampleIds = mydata$SubjectID)
#'
#' }
#' @export
rbioClass_svm_ncv_fs_v3 <- function(x, y,
   ...
   ## Validate and store sampleIds
   has_group_ids <- !is.null(sampleIds)
   if (has_group_ids) {
     if (length(sampleIds) != nrow(x)) {
       stop("sampleIds must have the same length as the number of rows in x.")
     }
     if (!is.character(sampleIds) && !is.factor(sampleIds) && !is.numeric(sampleIds)) {
       stop("sampleIds must be a character, factor, or numeric vector.")
     }
  ...
  if (verbose){
   ...
   out <- list(univariate.fs = univariate.fs,
               cv.fold = fold,
               randomized.sample.index = random_sample_idx,
               sampleIds = if (has_group_ids) sampleIds else NULL,
               sampleIdToFold = sample_id_to_fold,
               model.type = model_type,
               tot.nested.accuracy.summary = tot.nested.acc.summary,
               tot.nested.RMSE.summary = tot.nested.rmse.summary,
               tot.nested.rsq.summary = tot.nested.rsq.summary,
